...
namespace SudokuSolver.Strategies
{
    class NakedPairStrategy : ISudokuStrategy
    {
        ...
        {
            var valueToEliminateArray = valuesToEliminate.ToString().ToCharArray();
            foreach(var valueToEliminate in valueToEliminateArray)
            {
            ...
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    ...
                    var elementInSameBlock = _sudokuMapper.Find(givenRow, givenCol).StartRow == _sudokuMapper.Find(row, col).StartRow &&
                        _sudokuMapper.Find(row, col).StartCol == _sudokuMapper.Find(givenRow, givenCol).StartCol;
